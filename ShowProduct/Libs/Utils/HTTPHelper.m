...
-(void)beginPostRequest:(NSString*)url withDictionary:(NSDictionary*)postData
{
    ...
    [request setUserInfo:[NSDictionary dictionaryWithObject:url forKey:@"Url"]];//设置上下文的文件基本信息
...
-(void)beginRequest:(FileModel *)fileInfo isBeginDown:(BOOL)isBeginDown setAllowResumeForFileDownloads:(BOOL)allow
{
    ...
    NSDictionary* dict = [NSDictionary dictionaryWithObjectsAndKeys:fileInfo,@"File",fileURL,@"Url", nil];
    [request setUserInfo:dict];//设置上下文的文件基本信息
    [request setTimeOutSeconds:30.0f];
    if (isBeginDown) {
        [request startAsynchronous];
    }
...
-(void)requestFailed:(ASIHTTPRequest *)request
{
    ...
    else
    {
        ...
        [[NSNotificationCenter defaultCenter]postNotificationName:postKey object:error];
    }
}
...
//                NSLog(@"responseData: %@",responseData);
                NSLog(@"http requestFinished received");
            }
        }
        NSString* postKey = [request.userInfo objectForKey:@"Url"];
        [[NSNotificationCenter defaultCenter]postNotificationName:postKey object:postObject userInfo:dict];
        return;
    }
