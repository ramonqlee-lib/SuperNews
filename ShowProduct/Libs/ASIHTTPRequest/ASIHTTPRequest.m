...
#define kMaxConcurrentOperationCountForDownload 1
...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		[sharedQueue setMaxConcurrentOperationCount:kMaxConcurrentOperationCountForDownload];
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[ReachabilityAs reachabilityForInternetConnection] startNotifer];
...
{
	return ([[ReachabilityAs reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN);
}
