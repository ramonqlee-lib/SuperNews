...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(clearAllDisk)
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    NSData *image = [arguments objectForKey:kImageData];
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
    ...
    NSData *image = [NSData dataWithContentsOfFile:[self cachePathForKey:key]];
    if (image)
    {
        [mutableArguments setObject:image forKey:kImageData];
    }
